...
chcp 65001 >nul
cd /d "%~dp0"
echo === Publica drafturile pe GitHub Pages (o singura data) === > publica.log
echo.
...
gh repo create VladimirMalii/gral-preview --public --source=. --push --description "Drafturi de prezentare pagini GRAL Medical / OncoFort (noindex)" >> publica.log 2>&1
type publica.log
git remote get-url origin >nul 2>&1 || git remote add origin https://github.com/VladimirMalii/gral-preview.git
git push -u origin main >> publica.log 2>&1
echo.
echo 2. Activez GitHub Pages (ramura main, radacina)...
gh api -X POST repos/VladimirMalii/gral-preview/pages -f "source[branch]=main" -f "source[path]=/" >> publica.log 2>&1
gh api repos/VladimirMalii/gral-preview/pages >> publica.log 2>&1
echo.
...
echo.
echo    Tot ce s-a intamplat e scris in publica.log (Claude il citeste daca ceva nu merge).
echo.
pause
